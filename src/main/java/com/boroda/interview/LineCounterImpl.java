...
public class LineCounterImpl implements LineCounter {
    ...
    @Override
    public int calculateLinesInFolder(File folderOrFile) {
        int resultCount = 0;

        if (folderOrFile.isDirectory()) {
            for (File file : Objects.requireNonNull(folderOrFile.listFiles())) {
                if (file.isDirectory()) {
                    resultCount = resultCount + calculateLinesInFolder(file);
                } else {

                    try (BufferedReader br = Files.newBufferedReader(Paths.get(file.getAbsolutePath()))) {
                        resultCount = resultCount + calculateLinesOfCode(br);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }
            ...
            try (BufferedReader br = Files.newBufferedReader(Paths.get(folderOrFile.getAbsolutePath()))) {
                resultCount = calculateLinesOfCode(br);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    @Override
    public int calculateLinesOfCode(BufferedReader bReader) {
        int count = 0;
